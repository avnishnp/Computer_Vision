    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % LOG_INTERVAL == 0:
            ...
            train_losses.append(loss.item())
             # Compute the batch accuracy and append it to the train_counter list
            acc = 100. * correct / len(data)
            train_counter.append(acc)
            # Print the current batch loss and accuracy
...
def plot_accuracy(train_counter, train_losses, test_counter, test_losses):
...
def main(argv):
    ...
    plot_accuracy(train_counter, train_losses, test_counter, test_losses)
    # save the model
